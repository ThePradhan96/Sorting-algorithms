...
##created by - Priyanka Pradhan
 ...
 ##pancake sort problem by A* search algo
...
def backwardCost(stack):
	...
	for i in range(0, len(stack) - 1):
		distance = abs((stack[i] - stack[i + 1]) - 1)
		bkcost.append(distance)
	return bkcost


## cal. the cost of moving forward in a stack / heuristic function --- by subtracting one from it until it reaches the last number
def forwardCost(stack):
	distance = 0
	cost = 0
	fwdcost = []  ##fwc[2] = 1 , fwc[3] should be 0 

	for i in range(0, len(stack) - 1):
		distance = abs(stack[i] - stack[i + 1])
		### if there is more than one number left on either side of this current position- then move orelse not
		if distance > 1:
			cost = 1
		else:
			cost = 0 
...
##calculating the totalcost i.e sum of bkwrd + forward cost and store it
def totalCost(stack):
	## zip() method takes iterable and returns a single object
	tc = [i + k for i, k in zip(backwardCost(stack), forwardCost(stack))]
	##
	totalcostSum = sum(tc)
	return totalcostSum
...
if __name__ == '__main__':
	...
	main()
print('execution time for A star search : %s seconds' % (time.time() - start_time))
